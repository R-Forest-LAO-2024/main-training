...
## 2. Load data
tree_agb <- read_csv("data/tree_agb.csv")
...
## !! EX
## Create tp_comb and assign it a left_join() between 'treeplot' and 'tp_agb'
## !! 
...
## !! EX
## + Create 'tp_carbon' from tp_rs and use mutate() to calculate 
##   - `treeplot_bgb` with RS * AGB
##   - `treeplot_agb` with AGB * CF
##   - `treeplot_bgb` with (AGB + BGB) * CF
## !! 
tp_carbon <- tp_rs |>
  mutate(
    treeplot_bgb_ha = treeplot_agb_ha * rs,
