...
source("R/03-play-tree.R", local = TRUE)
source("R/03-play-spatial.R", local = TRUE)
source("R/04-calc-tree-agb-corr.R", local = TRUE)
source("R/05-aggregate-carbon-corr.R", local = TRUE)
...
![REDD+ carbon accounting steps](assets/Carbon accounting steps LaoPDR.png){#fig-ca}
...
Each strength was demonstrated with a different R project. The speed was showcased with running the NFI testing data in between timestamps recorders from the {tictoc} package. The whole calculation chain (loading data, a few visual checks, calculating biomass for live trees and deadwood, aggregation to plot, cluster and forest type) could run in less than 1 second.
